...
- **Q3 – Pares con rasgos Algo por Algo** (`question3_quid_pairs`):
  ...
  - **¿Cómo lo hace?** Primero normaliza todos los textos relevantes (descripciones, referencias, conceptos) para no perder coincidencias por tildes, mayúsculas o formatos diferentes. Después busca palabras de aprobación (autoriza, aprueba, firma, licitación, contrato, orden de compra…) y de compensación o favores (agradecido, incentivo, moche, coima, salida privada, trato especial…). Combina esas pistas con la jerarquía (si hay jefes involucrados), montos atípicos para la persona, pagos pegados a umbrales típicos, desfases entre carga y autorización y coincidencias de aprobación/compensación en ventanas cortas de tiempo. Si no encuentra suficientes pares con todo esto, relaja gradualmente los filtros y arma un ejemplo claro para cada dupla.
  - **Cómo leer sus columnas principales:**
    ...
    - `conceptos_detectados_en_los_movimientos`: lista los conceptos asociados a los movimientos sospechosos (por ejemplo, “licitación exprés”, “bono especial”), para que puedas revisar rápidamente qué temas aparecen en los textos.
